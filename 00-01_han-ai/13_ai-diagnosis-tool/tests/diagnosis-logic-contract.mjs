...
const check = Object.fromEntries(
  ['ok', 'equal', 'deepEqual', 'match', 'doesNotMatch'].map((method) => [
    method,
    (...args) => {
      assert[method](...args);
      checks++;
...
const questionsSource = source
  ...
  ?.split('\n\nconst STAGE_INFO')[0];
check.ok(questionsSource, 'QUESTIONS definition must remain readable');

const stageSource = source.match(/function getStage\(score\) \{[\s\S]*?\n\}/)?.[0];
check.ok(stageSource, 'getStage definition must remain readable');
...
check.equal(QUESTIONS.length, 12, 'the diagnosis must keep all 12 questions');
check.deepEqual(
...
for (const [score, expected] of [
  ...
  check.equal(getStage(score), expected, `stage boundary ${score}`);
}

check.match(source, /\/\.netlify\/functions\/generate-comment/,
  'the existing recommendation endpoint must remain unchanged');
check.doesNotMatch(source, /fetch\([^)]*https?:\/\//,
...
check.match(pricesHtml, /① AI活用知識編（90分）<\/th><td>60,000円<\/td><td>80,000円/);
check.match(pricesHtml, /⑤ 実践編 Claude Code特化（120分・グループMAX3名）<\/th><td colspan="2">180,000円/);
check.match(pricesHtml, /AI経営改善パッケージ（期間3か月　計16時間分）<\/th><td colspan="2">360,000円/);
check.match(detailHtml, /AI活用伴走・月2回\|100,000円\/月/);
check.match(detailHtml, /全コースセット（①②③④\+⑤グループ・10名）\|560,000円/);

console.log(`diagnosis logic contract: ${checks} checks PASS`);
